import { useRef } from "react";
import PropTypes from "prop-types";
import { Swiper, SwiperSlide } from "swiper/react";
import { Navigation, Autoplay } from "swiper/modules";

// Import Swiper styles
import "swiper/css";
import "swiper/css/navigation";

export default function TechArtemisSwiper({ slides }) {
  // Using refs for navigation ensures Swiper connects to these specific elements
  const prevRef = useRef(null);
  const nextRef = useRef(null);

  if (!slides?.length) return null;

  return (
    <div className="mx-auto relative w-full py-0">
      {/* Navigation Buttons */}
      <div className="flex justify-end flex-col md:flex-row absolute bottom-[-60px] left-1/2 -translate-x-1/2 transform md:relative md:left-0 md:translate-x-0 md:mb-5">
        <div className="flex justify-end items-center gap-5 z-10">
          <button 
            ref={prevRef}
            className="cursor-pointer hover:opacity-70 transition-opacity disabled:opacity-30" 
            aria-label="Previous Slide"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="45" height="15" viewBox="0 0 45 15" fill="none" className="rotate-180">
               {/* I flipped your first SVG's path or use rotate-180 to make it point left properly */}
               <path d="M43.7754 6.73909C44.1659 7.13415 44.1659 7.76731 43.7754 8.1533L37.4119 14.4434C37.0214 14.8294 36.3883 14.822 35.9978 14.427C35.6073 14.0319 35.6073 13.3987 35.9978 13.0128L41.6542 7.42157L35.9978 1.69905C35.6073 1.30399 35.6073 0.670827 35.9978 0.284836C36.3883 -0.101155 37.0214 -0.0938047 37.4119 0.301253L43.7754 6.73909ZM-0.000825914 6.93799L-0.000825871 5.93799L43.0684 6.43799L43.0684 7.43799L43.0684 8.43799L-0.000825958 7.93799L-0.000825914 6.93799Z" fill="#555555"/>
            </svg>
          </button>
          <button 
            ref={nextRef}
            className="cursor-pointer hover:opacity-70 transition-opacity disabled:opacity-30" 
            aria-label="Next Slide"
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="45" height="15" viewBox="0 0 45 15" fill="none">
              <path d="M0.292941 7.98894C-0.0975572 7.59388 -0.0975572 6.96071 0.292941 6.57472L6.65647 0.284639C7.04697 -0.101352 7.68009 -0.0940023 8.07059 0.301055C8.46109 0.696113 8.46109 1.32928 8.07059 1.71527L2.41412 7.30646L8.07059 13.029C8.46109 13.424 8.46109 14.0572 8.07059 14.4432C7.68009 14.8292 7.04697 14.8218 6.65647 14.4268L0.292941 7.98894ZM44.0692 7.79004L44.0692 8.79004L1 8.29004L1 7.29004L1 6.29004L44.0692 6.79004L44.0692 7.79004Z" fill="#555555"/>
            </svg>
          </button>
        </div>
      </div>

      <Swiper
        modules={[Navigation, Autoplay]}
        spaceBetween={20} // Added small space between slides
        slidesPerView={1}
        loop={true}
        autoplay={{
          delay: 5000, // Reduced from your 250s delay for testing!
          disableOnInteraction: false,
        }}
        navigation={{
          prevEl: prevRef.current,
          nextEl: nextRef.current,
        }}
        // This update is crucial when using React Refs for navigation
        onBeforeInit={(swiper) => {
          swiper.params.navigation.prevEl = prevRef.current;
          swiper.params.navigation.nextEl = nextRef.current;
        }}
        breakpoints={{
          768: { slidesPerView: 2 },
          1024: { slidesPerView: 2 },
        }}
        className="w-full"
      >
        {slides.map((slide, index) => (
          <SwiperSlide key={slide.id || index}>
            <div className="w-full group overflow-hidden">
              <div className="relative aspect-[97/120] w-full overflow-hidden bg-slate-900">
                <img
                  src={slide.img}
                  alt={slide.title}
                  className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent" />
                <div className="absolute inset-0 p-6 md:p-10 flex flex-col justify-end text-white">
                  <p className="text-base font-semibold uppercase mb-2">
                    {slide.title}
                  </p>
                  <p className="text-sm font-light opacity-90 line-clamp-3">
                    {slide.desc}
                  </p>
                </div>
              </div>
            </div>
          </SwiperSlide>
        ))}
      </Swiper>
    </div>
  );
}

TechArtemisSwiper.propTypes = {
  slides: PropTypes.arrayOf(
    PropTypes.shape({
      id: PropTypes.oneOfType([PropTypes.string, PropTypes.number]),
      title: PropTypes.string.isRequired,
      img: PropTypes.string.isRequired,
      desc: PropTypes.string,
    })
  ).isRequired,
};