document.addEventListener("DOMContentLoaded", () => {
      ...
      .map((item, index) => {
        ...
        return `
          <div class="flex gap-5 mb-8 cursor-pointer">
            ...
            <div class="blog_body">
              <p><a href="${newsUrl}" ${
          hasCustomLink ? 'target="_blank" rel="noopener noreferrer"' : ""
        }> ${item.title}</a></p>
      ...
      .map(({ node }) => {
        ...
        return `
          <li class="py-2 cursor-pointer mb-2" data-slug="${node.slug}">
            <a style="display:block;width:100%;padding:15px;${style}">${node.name}</a>
          </li>
        `;
      })
      .join("");
  ...
  function createNewsCardHtml(item) {
    ...
    return `
      ...
      hasCustomLink ? 'target="_blank" rel="noopener noreferrer"' : ""
        ...
        <div class="blog-card border-b border-dotted pb-4 sm:pb-4 pr-4 sm:border-r last:border-b-0 last:sm:border-r-0">
  ...
  function createMediaKitHtml() {
    const fileSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="38" height="34" viewBox="0 0 106 95" fill="none">
    ...
    <defs>
    <linearGradient id="paint0_linear_584_313" x1="18.2328" y1="98.9584" x2="46.3383" y2="55.8931" gradientUnits="userSpaceOnUse"><stop stop-color="#F58220"/><stop offset="1" stop-color="#F2728C"/></linearGradient>
    <linearGradient id="paint1_linear_584_313" x1="117.468" y1="-12.6698" x2="55.7173" y2="57.9732" gradientUnits="userSpaceOnUse"><stop stop-color="#F58220"/><stop offset="1" stop-color="#F2728C"/></linearGradient>
    <linearGradient id="paint2_linear_584_313" x1="-26.2688" y1="-28.7571" x2="94.0919" y2="107.009" gradientUnits="userSpaceOnUse"><stop stop-color="#0074BC"/><stop offset="1" stop-color="#44C8F5"/></linearGradient>
    </defs></svg>`;
    const downloadBtn = (link) => `
      <a target="_blank" href="${link}" class="contact_cta flex items-center gap-2 text-white uppercase pw-2 mb-2" data-astro-cid-qmpwvs2w="">
        <span style="min-width: 88px;" data-text="Download" data-astro-cid-qmpwvs2w="">Download</span>
        <img src="https://vivrithi.vercel.app/Vivrithi_logo.svg" class="w-5 h-5" data-astro-cid-qmpwvs2w="">
      </a>`;

    const mediaKitItems = [
      { name: "Image of Vineet Sukumar - Founder & MD", link: "https://vivritinextdev.wpenginepowered.com/wp-content/uploads/2025/12/Vineet-recent-photo.jpg" },
      { name: "Profile of Vineet Sukumar - Founder & MD", link: "https://vivritinextdev.wpenginepowered.com/wp-content/uploads/2026/01/Vineet-Sukumar-Profile.docx" },
      { name: "VNL Logo", link: "https://vivritinextdev.wpenginepowered.com/wp-content/uploads/2026/01/VNL-Logo.zip" },
      { name: "VNL Brand Guidelines", link: "https://vivritinextdev.wpenginepowered.com/wp-content/uploads/2025/12/VNL-Brand-Guidelines.pdf" },
    ];

    return `
      <h1 class="gradient-text text-[28px] md:text-[35px] font-heading leading-[120%] mb-4">Media Kit</h1>
      <ul class="list_of_files">
        ${mediaKitItems.map(item => `
          <li>
            <div style="display:flex;align-item:center;gap:10px;">
              ${fileSvg}
              <a target="_blank" href="${item.link}" download>${item.name}</a>
            </div>
            ${downloadBtn(item.link)}
          </li>
        `).join('')}
      </ul>
    `;
  ...
  function renderPage(page) {
    ...
    if (activeCategory === "media-kit") {
      ...
      container.innerHTML = createMediaKitHtml();
    ...
    container.innerHTML = list.map(createNewsCardHtml).join("");
    renderPagination();
  }
  ...
  function renderPagination() {
    ...
    // Previous text
    wrapper.appendChild(createBtn("Previous", currentPage - 1, currentPage === 1, "px-2 lg:px-8 py-2 hidden sm:block"));
    ...
    const leftArrowSvg = `<span class="rotate-180"><svg xmlns="http://www.w3.org/2000/svg" width="11" height="15" viewBox="0 0 11 15" fill="none"><path d="M1 13.5001L9 7.00006L1 1.00006" stroke="white" stroke-width="2" stroke-linecap="round"></path></svg></span>`;
    wrapper.appendChild(createBtn(leftArrowSvg, currentPage - 1, currentPage === 1, leftArrowClass));
    ...
    pagesToShow.forEach((p) => {
      ...
      const btnClass = p === currentPage ? "border-gradent border-gradent-active" : "border-gradent";
      wrapper.appendChild(createBtn(p, p, false, btnClass));
      lastRendered = p;
    });
    ...
    const rightArrowSvg = `<span><svg xmlns="http://www.w3.org/2000/svg" width="11" height="15" viewBox="0 0 11 15" fill="none"><path d="M1 13.5001L9 7.00006L1 1.00006" stroke="white" stroke-width="2" stroke-linecap="round"></path></svg></span>`;
    wrapper.appendChild(createBtn(rightArrowSvg, currentPage + 1, currentPage === totalPages, rightArrowClass));

    // Next text
    wrapper.appendChild(createBtn("Next", currentPage + 1, currentPage === totalPages, "px-2 lg:px-8 py-2 hidden sm:block"));
  }
  ...
  async function loadInitial() {
    try {
      ...
      // Initial filter based on activeCategory
      filterAndRender();
